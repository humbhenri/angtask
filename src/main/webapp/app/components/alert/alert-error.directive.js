(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('angtaskApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-angtaskApp-error');
                var entityKey = httpResponse.headers('X-angtaskApp-params');
                if (errorHeader) {
